...
class PathOptimizer:
    ...
    def thetaf(self, a, b, c, d, s):
        ...
        # ...
        thetas = np.multiply(a,s)+np.multiply(b,(s**2)*0.5)+np.multiply(c,(s**3)/3)+np.multiply(d,(s**4)/4)
        return thetas
        # ------------------------------------------------------------------
    ...
    def sample_spiral(self, p):
        ...
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        t_points = self.thetaf(self, a, b, c, d, s_points)
        x_points = scipy.integrate.cumtrapz(cos(t_points), x=s_points, initial=None)
        y_points = scipy.integrate.cumtrapz(sin(t_points), x=s_points, initial=None)
